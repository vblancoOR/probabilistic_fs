...
def pfs_kappa(train, C, n_paths, mu_path, p, p_plus, trees_vars, trees_vals, tree_depth,
    leaf_classes, desired_class, epsilon, timelimit):
    ...
    for r in R:
        for i in leaf_nodes:
            u_order[r,i] = m.addVar(lb=0,ub=1,vtype=GRB.CONTINUOUS,
                                    name='aplha'+ str(r) + '_' + str(i))
            for l in leaf_nodes:
                alpha_order[r,i,l] = m.addVar(vtype=GRB.BINARY,
                                        name='aplha'+ str(r) + '_' + str(i)
                                        + '_' + str(l))
    ...
    m.Params.OptimalityTol = 0.0000001
    m.Params.IntFeasTol = 0.0000001
    m.update()
    ...
    for r in R:
        for i in leaf_nodes:
            for l in leaf_nodes:
                m.addConstr(u_order[r,i] <= theta[r,l] + (1-alpha_order[r,i,l]),
                        name="C_ord1[%d,%d,%d]"%(r+1,i+1,l+1))

    for r in R:
        for i in leaf_nodes:
            m.addConstr(quicksum(alpha_order[r,i,l] for l in leaf_nodes) == 1,
                        name="C_ord2[%d,%d]"%(r+1,i+1))
        for l in leaf_nodes:
            m.addConstr(quicksum(alpha_order[r,i,l] for i in leaf_nodes) == 1,
                        name="C_ord3[%d,%d]"%(r+1,l+1))

    for r in R:
        for i in leaf_nodes[:(len(leaf_nodes)-1)]:
            m.addConstr(u_order[r,i]  <= u_order[r,i+1] ,
                        name="C_ord4[%d,%d]"%(r+1,i+1))

    for r in R:
        m.addConstr(quicksum(u_order[r,leaf_nodes[i]] for i in range(n_paths)) >= mu_path,
                    name="C_ord5[%d]"%(r+1))

    for r in R:
        m.addGenConstrLog(u_order[r,leaf_nodes[n_paths]], log_sumvar[r],
                          name="Caux5[%d]"%(r+1))
