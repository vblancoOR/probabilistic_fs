...
def pfs_min(train, C, p, p_plus, trees_vars, trees_vals, tree_depth,
    ...
    for r in R:
        ...
        ye[r] =  m.addVar(vtype=GRB.CONTINUOUS, name='ye'+str(r))
        ...
        for t in leaf_nodes:
            ...
            q[r,t] = m.addVar(vtype=GRB.BINARY,name='q' + str(r) +
                                                       '_' + str(t))

            pi[r,t] = m.addVar(lb=0,ub=1,vtype=GRB.CONTINUOUS,name='pi' +
                                               str(r) + '_' + str(t))
            ...
            for d in D:
                ...
                if d == (tree_depth-1):
                    m.addConstr(alpha[r,t,d]==omega[r,get_parent(t,tree_depth),t],
                                        name="Caux2b[%d,d%d,%d]"%(r+1,t+1,d+1))

                else:
                    m.addConstr(alpha[r,t,d]<=pp*alpha[r,t,d+1]+
                                gamma[r,t,d]*(pp_plus-pp),
                                        name="Caux2c[%d,d%d,%d]"%(r+1,t+1,d+1))
                    m.addConstr(gamma[r,t,d]<=beta[r, trees_vars.iloc[r,index-1]],
                                        name="Caux2d[%d,d%d,%d]"%(r+1,t+1,d+1))
                    m.addConstr(gamma[r,t,d]<=alpha[r,t,d+1] ,
                                        name="Caux2e[%d,d%d,%d]"%(r+1,t+1,d+1))


    for r in R:
        for t in leaf_nodes:
            m.addConstr(theta[r,t] <= big_alpha[r,t] + phi[r,t] + delta[r],
                    name="Caux3[d%d,%d]"%(r+1,t+1))

    # epsilon_2 can be set > 0 to modify O.F tolerance
    epsilon_2 = 0.00001
    for r in R:
        for l in leaf_nodes:
            m.addConstr(pi[r,l] <= theta[r,l] + (1-q[r,l]) , name="Caux4a[%d,%d]"%(r+1,l+1))
            m.addConstr(pi[r,l] >= epsilon_2 - q[r,l], name="Caux4a[%d,%d]"%(r+1,l+1))
            m.addConstr(pi[r,l] <= epsilon_2 + q[r,l], name="Caux4a[%d,%d]"%(r+1,l+1))
            m.addConstr(theta[r,l] <= epsilon_2 + q[r,l], name="Caux4a[%d,%d]"%(r+1,l+1))
            m.addConstr(theta[r,l] >= epsilon_2 * q[r,l], name="Caux4a[%d,%d]"%(r+1,l+1))

    for r in R:
        for l in leaf_nodes:
            m.addConstr(ye[r] <= pi[r,l], name="Caux5a[%d,%d]"%(r+1,l+1))
            m.addConstr(ye[r] >= pi[r,l] - (1-z[r,l]), name="Caux5b[%d,%d]"%(r+1,l+1))

    for r in R:
        m.addConstr(sum_var[r]== ye[r] ,
                            name="Caux6[%d]"%(r+1))

    for r in R:
        m.addGenConstrLog(sum_var[r], log_sumvar[r],name="Caux7[%d]"%(r+1))
